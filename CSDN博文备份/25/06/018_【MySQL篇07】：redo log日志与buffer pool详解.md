...
> 原创 已于 2025-08-12 09:00:16 修改 · 公开 · 1.2k 阅读 · 30 · 22 · CC 4.0 BY-SA版权 版权声明：本文为博主原创文章，遵循 CC 4.0 BY-SA 版权协议，转载请附上原文出处链接和本声明。
...
> - 当你执行一个更新操作时，InnoDB 存储引擎会先将这个更新操作应用到 **Buffer Pool** 中的数据页（产生脏页）。
> - 然后，再将操作记录到 **redo log buffer (重做日志缓存)** 中。
...
>  **这个过程非常重要：先写日志，再更新内存（由图一也可清晰地看出）。** 这就是所谓的 **WAL (Write-Ahead Logging，先写日志) 技术** 。
